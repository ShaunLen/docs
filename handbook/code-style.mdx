...
<Tabs>
  <Tab title="Types">
    Use **PascalCase**. Names should be descriptive and unambiguous.

    | Good | Bad |
    |------|-----|
    | `InteractionProvider` | `Manager` |
    | `InputContextStack` | `Utils` |
    | `WeaponDefinition` | `Data2` |
  </Tab>
  <Tab title="Methods">
    Use **PascalCase**. Name methods as actions.

    | Good | Bad |
    |------|-----|
    | `ApplyInput` | `Handle` |
    | `ValidateConfiguration` | `DoThing` |
    | `RegisterInteraction` | `Process` |
  </Tab>
  <Tab title="Variables">
    Use **camelCase**. Describe intent, not mechanics.
  ...
  <Tab title="Booleans">
    Name so they read naturally in conditionals.
